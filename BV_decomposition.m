...
[testT, testX] = libsvmread('../data/a7a.t');
Data1 = [trainT trainX];
Data2 = [testT testX];
...
accCell = cell(length(C_vector),length(numAlpha0_vector));
% try 
    ...
    for i = 1:1:length(C_vector)
        for j = 1:1:length(numAlpha0_vector)
            ...
            for k = 1:1:n
                ...
                trainT = D_n{k}(:,1);
                if(numAlpha0 > 0)
                    genAlpha0(trainT, trainX, numAlpha0, C, kerType, 0);
                end
                [alpha, b] = svmTrain(trainT, trainX, numAlpha0, C, kerType, 0);
                predT(:,k) = svmPredict(alpha, b, trainT, trainX, numAlpha0, testX, kerType, 0);
